...
const functionalTests = [
  ...
  { id:'TC064', module:'API', page:'Chat API', scenario:'GET /api/chat returns messages', expected:'API returns messages for given roomId from Supabase', actual:'GET endpoint returns hardcoded empty array: const data: any[] = []. No Supabase query.', status:'Fail', screenshot:'API_Chat_EmptyData.png', reason:'Chat API returns empty array regardless of roomId - no DB query', improvement:'Implement Supabase query to fetch messages by roomId' },
  { id:'TC065', module:'API', page:'Jobs API', scenario:'GET /api/jobs returns jobs', expected:'API returns jobs with category/pagination filters from Supabase', actual:'GET endpoint returns hardcoded empty array: const data: any[] = []. No Supabase query.', status:'Fail', screenshot:'API_Jobs_EmptyData.png', reason:'Jobs API returns empty data - DB query not implemented', improvement:'Implement Supabase query to fetch jobs with filters' },
  { id:'TC066', module:'API', page:'Workers API', scenario:'GET /api/workers returns workers', expected:'API returns worker profiles from Supabase', actual:'Contains TODO comments. Both GET and POST return empty arrays. No Supabase query.', status:'Fail', screenshot:'API_Workers_NotImplemented.png', reason:'Worker API is a stub with TODO comments', improvement:'Implement Supabase query to worker_profiles table' },
  { id:'TC067', module:'API', page:'Listings API', scenario:'GET /api/listings returns listings', expected:'API returns marketplace listings from Supabase', actual:'Contains TODO comments. Returns empty data. No Supabase query.', status:'Fail', screenshot:'API_Listings_NotImplemented.png', reason:'Listings API is a stub with TODO comments', improvement:'Implement Supabase query to listings table' },
  { id:'TC068', module:'API', page:'Bookings API', scenario:'GET /api/bookings returns bookings', expected:'API returns bookings from Supabase', actual:'Contains TODO comments. Returns empty data. No Supabase query.', status:'Fail', screenshot:'API_Bookings_NotImplemented.png', reason:'Bookings API is a stub with TODO comments', improvement:'Implement Supabase query to bookings table' },
  { id:'TC069', module:'API', page:'QR Verify API', scenario:'POST /api/qr/verify validates QR codes', expected:'API validates QR code against stored codes in database', actual:'Always returns {verified: true} without any database verification. Contains TODO comment.', status:'Fail', screenshot:'API_QRVerify_AlwaysTrue.png', reason:'SECURITY FLAW: QR verification always returns true without checking DB', improvement:'Implement real QR code validation against Supabase stored codes' },
  { id:'TC070', module:'API', page:'Payments API', scenario:'POST /api/payments processes payment', expected:'Payment processed through Stripe/Adyen gateway', actual:'Returns mock payment with status "pending". Contains TODO: "Integrate Stripe/Adyen". No real payment processing.', status:'Fail', screenshot:'API_Payments_MockOnly.png', reason:'No payment gateway integration - returns mock data', improvement:'Integrate Stripe or Adyen PCI-compliant payment provider' },

  // Data Persistence Issues (Critical)
  { id:'TC071', module:'Profile', page:'Profile Edit', scenario:'Avatar upload persists across sessions', expected:'Avatar saved to Supabase storage bucket', actual:'Avatar upload uses FileReader to store as DataURL in localStorage only. Not connected to Supabase storage.', status:'Fail', screenshot:'Profile_AvatarNotPersisted.png', reason:'Avatar stored in localStorage, not Supabase storage. Lost on browser clear.', improvement:'Connect to uploadAvatar in supabase.ts for persistent storage' },
  { id:'TC072', module:'Community', page:'Community', scenario:'Created group persists in database', expected:'New group saved to communities table in Supabase', actual:'handleCreate only validates with AI safety scan and shows success message. Does NOT call createCommunity from supabase.ts.', status:'Fail', screenshot:'Community_GroupNotSaved.png', reason:'Group creation UI does not call database save function', improvement:'Call createCommunity after successful safety scan' },
  { id:'TC073', module:'Community', page:'Community', scenario:'Group post persists in database', expected:'Post saved to posts table with community context', actual:'handlePost scans content but only updates local state. Does NOT call createPost.', status:'Fail', screenshot:'Community_PostNotSaved.png', reason:'Community post not saved to database', improvement:'Call createPost with community_id after content moderation passes' },
  { id:'TC074', module:'Friends', page:'Friends', scenario:'Friend requests persist', expected:'Friend request saved to friend_requests table', actual:'All friend request operations (send/accept/decline) only modify local component state. No Supabase calls.', status:'Fail', screenshot:'Friends_RequestsNotPersisted.png', reason:'Friend operations not connected to backend', improvement:'Call sendFriendRequest/respondFriendRequest from supabase.ts' },
  { id:'TC075', module:'Friends', page:'Friends', scenario:'Blocked users enforced server-side', expected:'Blocked users stored in friend_requests table with blocked status', actual:'Block/unblock only uses localStorage via getBlockedUsers/toggleBlock. Not server-enforced.', status:'Fail', screenshot:'Friends_BlockingClientOnly.png', reason:'Blocking is client-side only, can be bypassed via API', improvement:'Use respondFriendRequest with blocked status and enforce via RLS' },
  { id:'TC076', module:'Wallet', page:'Wallet', scenario:'Wallet shows real balance', expected:'Balance fetched from wallets table in Supabase', actual:'All balance and transaction data is hardcoded in useState. No Supabase calls.', status:'Fail', screenshot:'Wallet_DummyData.png', reason:'Wallet uses hardcoded mock data, not connected to database', improvement:'Call getWalletBalance and getTransactions from supabase.ts' },
  { id:'TC077', module:'Chat', page:'Chat', scenario:'Messages persist across sessions', expected:'Messages saved to messages table via sendMessage', actual:'Messages stored in component state only. No API calls to save. Auto-replies are hardcoded.', status:'Fail', screenshot:'Chat_MessagesNotPersisted.png', reason:'Chat messages are ephemeral - lost on page refresh', improvement:'Call sendMessage from supabase.ts and subscribe to realtime updates' },
  { id:'TC078', module:'Search', page:'Search', scenario:'Search queries real database content', expected:'Search indexes content from Supabase tables', actual:'Search index built from hardcoded DEMO_WORKERS and DEMO_JOBS arrays only. No real DB content.', status:'Fail', screenshot:'Search_DemoDataOnly.png', reason:'Search only returns demo/mock data, not real database content', improvement:'Build search index from actual Supabase queries' },
  { id:'TC079', module:'Search', page:'Search', scenario:'Voice search captures real speech', expected:'Web Speech API captures and converts voice to search query', actual:'Voice button shows "Listening..." for 2 seconds then hardcodes query to "plumber". No real speech recognition.', status:'Fail', screenshot:'Search_VoiceSearchFake.png', reason:'Voice search is simulated - always returns "plumber" after timeout', improvement:'Implement real Web Speech API SpeechRecognition integration' },
