"use client";
export const dynamic = "force-dynamic";
import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { useThemeStore } from '@/store/useThemeStore';
import { getTheme } from '@/lib/theme';
import { signInWithGoogle, getSession } from '@/lib/supabase';
import { secureSignIn, secureSignUp, getPasswordStrength, generateDeviceFingerprint } from '@/lib/auth';
import { sanitizeHTML, validateInput, validators } from '@/lib/security';

export default function LoginPage() {
  const router = useRouter();
  const { isDark, toggle } = useThemeStore();
  const t = getTheme(isDark);
  const [mode, setMode] = useState<'login'|'signup'>('login');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [name, setName] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [showPw, setShowPw] = useState(false);
  const [pwStrength, setPwStrength] = useState({ score:0, feedback:'' });
  const [fieldErrors, setFieldErrors] = useState<Record<string,string>>({});
  const [loginAttempts, setLoginAttempts] = useState(0);
  const [lockoutUntil, setLockoutUntil] = useState(0);

  useEffect(() => {
    getSession().then(({ data }) => { if (data.session) router.push('/home'); });
  }, []);

  // Password strength indicator (signup only)
  useEffect(() => {
    if (mode === 'signup' && password) setPwStrength(getPasswordStrength(password));
  }, [password, mode]);

  // Lockout check
  const isLockedOut = Date.now() < lockoutUntil;

  const validate = (): boolean => {
    const errs: Record<string,string> = {};
    const emailErr = validateInput(email, validators.email);
    if (emailErr) errs.email = emailErr;
    const pwErr = validateInput(password, validators.password);
    if (pwErr) errs.password = pwErr;
    if (mode === 'signup') {
      const nameErr = validateInput(name, validators.displayName);
      if (nameErr) errs.name = nameErr;
      if (pwStrength.score < 2) errs.password = pwStrength.feedback;
    }
    setFieldErrors(errs);
    return Object.keys(errs).length === 0;
  };

  const validateField = (field: string) => {
    let err = '';
    if (field === 'email') err = validateInput(email, validators.email) || '';
    else if (field === 'password') err = validateInput(password, validators.password) || '';
    else if (field === 'name') err = validateInput(name, validators.displayName) || '';
    setFieldErrors(prev => err ? { ...prev, [field]: err } : (({ [field]: _, ...rest }) => rest)(prev));
  };

  const handleSubmit = async () => {
    setError('');
    if (isLockedOut) { setError(`Too many attempts. Try again in ${Math.ceil((lockoutUntil - Date.now())/1000)}s`); return; }
    if (!validate()) return;
    setLoading(true);
    try {
      if (mode === 'login') {
        const { data, error: err } = await secureSignIn(email, password);
        if (err) {
          const newAttempts = loginAttempts + 1;
          setLoginAttempts(newAttempts);
          if (newAttempts >= 5) {
            setLockoutUntil(Date.now() + 60000); // 1 min lockout
            setLoginAttempts(0);
            throw { message: 'Too many failed attempts. Account locked for 60 seconds.' };
          }
          throw err;
        }
        setLoginAttempts(0);
      } else {
        const sanitizedName = sanitizeHTML(name.trim());
        const { data, error: err } = await secureSignUp(email, password, sanitizedName);
        if (err) throw err;
      }
      // Redirect to stored destination or home
      const redirect = typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('auth_redirect') : null;
      if (redirect) { sessionStorage.removeItem('auth_redirect'); router.push(redirect); }
      else router.push('/home');
    } catch (e: any) { setError(e.message || 'Something went wrong'); }
    setLoading(false);
  };

  const strengthColor = pwStrength.score <= 1 ? '#ef4444' : pwStrength.score <= 2 ? '#f59e0b' : pwStrength.score <= 3 ? '#22c55e' : '#10b981';
  const strengthPct = (pwStrength.score / 5) * 100;

  return (
    <div style={{ background: isDark ? 'linear-gradient(180deg,#0a0a16,#0f0f1e,#121225)' : 'linear-gradient(180deg,#f8f9ff,#eef0ff)', color: t.text, minHeight: '100vh' }} className="flex items-center justify-center p-4">
      <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse at 50% 30%, ${t.accent}08 0%, transparent 60%)` }} />
      <div className="relative w-full max-w-md">
        {/* Header */}
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => router.push('/')}>
            <img src="/logo-128.png" alt="Datore" style={{ width:40, height:40, borderRadius:12, boxShadow:'0 2px 10px rgba(99,102,241,0.3)' }} />
            <span className="font-extrabold text-xl" style={{ background:'linear-gradient(135deg,#6366f1,#a78bfa)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>Datore</span>
          </div>
          <button onClick={toggle} className="glass-button p-2.5 rounded-xl" style={{ background:t.surface }}>{isDark ? '☀️' : '🌙'}</button>
        </div>

        {/* Card */}
        <div className="glass-card rounded-3xl p-7" style={{ background:t.card, borderColor:t.cardBorder, boxShadow: isDark ? '0 20px 60px rgba(0,0,0,0.3)' : '0 20px 60px rgba(0,0,0,0.08)' }}>
          <h2 className="text-2xl font-bold mb-1">{mode === 'login' ? 'Welcome Back' : 'Create Account'}</h2>
          <p className="text-sm mb-6" style={{ color:t.textSecondary }}>{mode === 'login' ? 'Sign in to your secure account' : 'Join the verified community marketplace'}</p>

          {/* Google OAuth */}
          <button onClick={() => signInWithGoogle()} className="w-full glass-button flex items-center justify-center gap-2.5 py-3 rounded-xl mb-4 font-medium text-sm" style={{ background:t.surface, color:t.text, borderColor:t.cardBorder }}>
            <svg width="18" height="18" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
            Continue with Google
          </button>

          <div className="flex items-center gap-3 my-5"><div className="flex-1 h-px" style={{ background:t.cardBorder }} /><span className="text-xs" style={{ color:t.textMuted }}>or</span><div className="flex-1 h-px" style={{ background:t.cardBorder }} /></div>

          {/* Signup: Name */}
          {mode === 'signup' && (
            <div className="mb-3">
              <input value={name} onChange={e => setName(e.target.value)} onBlur={() => validateField('name')} placeholder="Full Name" className="glass-input w-full px-4 py-3 rounded-xl text-sm" style={{ background:t.input, color:t.text, borderColor:fieldErrors.name ? '#ef4444' : t.inputBorder }} />
              {fieldErrors.name && <p className="text-[10px] mt-1" style={{ color:'#ef4444' }}>⚠ {fieldErrors.name}</p>}
            </div>
          )}

          {/* Email */}
          <div className="mb-3">
            <input value={email} onChange={e => setEmail(e.target.value)} onBlur={() => validateField('email')} type="email" placeholder="Email" autoComplete="email" className="glass-input w-full px-4 py-3 rounded-xl text-sm" style={{ background:t.input, color:t.text, borderColor:fieldErrors.email ? '#ef4444' : t.inputBorder }} />
            {fieldErrors.email && <p className="text-[10px] mt-1" style={{ color:'#ef4444' }}>⚠ {fieldErrors.email}</p>}
          </div>

          {/* Password with show/hide */}
          <div className="mb-1 relative">
            <input value={password} onChange={e => setPassword(e.target.value)} onBlur={() => validateField('password')} type={showPw ? 'text' : 'password'} placeholder="Password" autoComplete={mode==='login'?'current-password':'new-password'} className="glass-input w-full px-4 py-3 rounded-xl text-sm pr-12" style={{ background:t.input, color:t.text, borderColor:fieldErrors.password ? '#ef4444' : t.inputBorder }} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
            <button onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs" style={{ color:t.textMuted }}>{showPw ? '🙈' : '👁️'}</button>
          </div>
          {fieldErrors.password && <p className="text-[10px] mt-1 mb-2" style={{ color:'#ef4444' }}>⚠ {fieldErrors.password}</p>}

          {/* Password strength bar (signup) */}
          {mode === 'signup' && password && (
            <div className="mb-4 mt-2">
              <div className="h-1.5 rounded-full overflow-hidden" style={{ background:isDark?'rgba(255,255,255,0.06)':'rgba(0,0,0,0.06)' }}>
                <div className="h-full rounded-full transition-all duration-300" style={{ width:`${strengthPct}%`, background:strengthColor }} />
              </div>
              <p className="text-[10px] mt-1" style={{ color:strengthColor }}>{pwStrength.feedback}</p>
            </div>
          )}

          {/* Error display */}
          {error && (
            <div className="mb-3 p-3 rounded-xl flex items-center gap-2 text-xs" style={{ background:'rgba(239,68,68,0.1)', color:'#ef4444', border:'1px solid rgba(239,68,68,0.2)' }}>
              <span>⚠️</span> {error}
            </div>
          )}

          {/* Lockout warning */}
          {isLockedOut && (
            <div className="mb-3 p-3 rounded-xl text-xs text-center" style={{ background:'rgba(239,68,68,0.08)', color:'#ef4444' }}>
              🔒 Account temporarily locked. Too many failed attempts.
            </div>
          )}

          {/* Submit */}
          <button onClick={handleSubmit} disabled={loading || isLockedOut} className="btn-accent w-full py-3.5 rounded-xl text-sm mt-2" style={{ opacity:loading||isLockedOut?0.5:1 }}>
            {loading ? '⏳ Please wait...' : mode === 'login' ? '🔐 Sign In Securely' : '🚀 Create Account'}
          </button>

          {/* Security badge */}
          <div className="flex items-center justify-center gap-1.5 mt-4">
            <span className="text-[9px]" style={{ color:t.textMuted }}>🔒 256-bit encrypted · Device verified · Session audited</span>
          </div>

          {/* Toggle mode */}
          <p className="text-center text-sm mt-5" style={{ color:t.textSecondary }}>
            {mode === 'login' ? "Don't have an account?" : 'Already have an account?'}{' '}
            <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setFieldErrors({}); }} className="font-semibold" style={{ color:t.accent }}>{mode === 'login' ? 'Sign Up' : 'Sign In'}</button>
          </p>
        </div>

        {/* Footer */}
        <p className="text-center text-[10px] mt-6" style={{ color:t.textMuted }}>
          By continuing, you agree to Datore's <a href="/terms" style={{ color:t.accent }}>Terms</a> and <a href="/privacy" style={{ color:t.accent }}>Privacy Policy</a>
        </p>
      </div>
    </div>
  );
}
