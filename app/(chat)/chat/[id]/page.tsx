...
export default function ChatPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const msgs = await getChatMessages(roomId as string);
      setMessages(msgs);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "var(--bg-primary)" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12, background: "var(--bg-secondary)" }}>
        <button onClick={() => router.push("/messages")} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-primary)" }}><ArrowLeft size={20} /></button>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
        {messages.map((m: any) => (
          <div key={m.id} style={{ display: "flex", justifyContent: m.sender_id === userId ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: 16, fontSize: 14, lineHeight: 1.5, background: m.sender_id === userId ? "var(--accent)" : "var(--bg-card)", color: m.sender_id === userId ? "#0A0A0F" : "var(--text-primary)", borderBottomRightRadius: m.sender_id === userId ? 4 : 16, borderBottomLeftRadius: m.sender_id !== userId ? 4 : 16 }}>{m.content}</div>
          </div>
        ))}
        <div ref={messagesEnd} />
      </div>
      <div style={{ padding: "10px 16px", borderTop: "1px solid var(--border)", display: "flex", gap: 8, background: "var(--bg-secondary)" }}>
        ...
        <button onClick={handleSend} style={{ padding: "12px 16px", background: "var(--accent)", border: "none", borderRadius: "var(--radius-sm)", cursor: "pointer" }}><Send size={18} color="#0A0A0F" /></button>
      </div>
    </div>
  );
}
