...
export default function HomePage() {
  ...
  const actions = [
    ...
    { icon:'W', label:'Workers', path:'/jobplace/providers', bg:'#22c55e' },
    { icon:'M', label:'Map', path:'/jobplace/map', bg:'#f59e0b' },
    { icon:'S', label:'Market', path:'/marketplace', bg:'#ec4899' },
    { icon:'$', label:'Wallet', path:'/wallet', bg:'#8b5cf6' },
    { icon:'F', label:'Friends', path:'/friends', bg:'#06b6d4' },
    { icon:'Q', label:'QR Verify', path:'/qr-verify', bg:'#0ea5e9' },
    { icon:'V', label:'Safety', path:'/safety', bg:'#22c55e' },
  ];

  return (
    <div className="space-y-4 animate-fade-in max-w-2xl mx-auto">
      {/* Hidden file inputs - REAL file pickers */}
      ...
      <div className="glass-card rounded-2xl p-4 flex items-center gap-3" style={{ background:`linear-gradient(135deg,${t.accent}22,#8b5cf622)`, borderColor:t.cardBorder }}>
        ...
        <div>
          ...
          <h1 className="text-xl font-bold">Hello, {prefs.name || 'there'}!</h1>
        </div>
      </div>

      {/* Create Post Bar - buttons trigger REAL file pickers */}
            ...
            <div key={post.id} className="glass-card rounded-2xl p-4" style={{ background:t.card, borderColor:t.cardBorder }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px]" style={{ color:t.textMuted }}>{post.time}{post.type !== 'text' ? ` - ${post.type}` : ''}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 mt-3 pt-3" style={{ borderTop:`1px solid ${t.cardBorder}` }}>
                <button onClick={() => handleLike(post.id)} className="text-xs" style={{ color:likedPosts.includes(post.id)?'#ef4444':t.textMuted }}>
      ...
      {/* Create Post Modal - with REAL file upload */}
      {showPost && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }} onClick={() => { setShowPost(false); clearMedia(); }}>
          <div onClick={e => e.stopPropagation()} className="w-full max-w-md rounded-2xl p-5 space-y-4" style={{ background:isDark?'#1a1a2e':'#fff', border:`1px solid ${t.cardBorder}` }}>
            ...
            {/* Media area - REAL camera/gallery with preview */}
            {postType !== 'text' && (
                ...
                {mediaPreview ? (
                  <div className="relative rounded-xl overflow-hidden" style={{ border:`1px solid ${t.cardBorder}` }}>
                    ...
                    <div className="absolute top-2 right-2">
                      <button onClick={clearMedia} className="px-3 py-1 rounded-lg text-[11px] font-bold text-white" style={{ background:'rgba(0,0,0,0.7)' }}>X Remove</button>
                    </div>
                    <div className="px-3 py-1.5" style={{ background:isDark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.03)' }}>
                      <p className="text-[10px]" style={{ color:'#22c55e' }}>Attached: {mediaName}</p>
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => cameraRef.current?.click()} className="h-28 rounded-xl flex flex-col items-center justify-center" style={{ background:isDark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.02)', border:`2px dashed ${t.accent}55` }}>
                      <div className="w-10 h-10 rounded-full flex items-center justify-center mb-2" style={{ background:`${t.accent}22` }}>
                        <span className="text-sm font-bold" style={{ color:t.accent }}>{postType==='photo'?'CAM':'REC'}</span>
                      </div>
                      <p className="text-xs font-semibold" style={{ color:t.accent }}>{postType==='photo'?'Take Photo':'Record Video'}</p>
                      <p className="text-[10px]" style={{ color:t.textMuted }}>Open camera</p>
                    </button>
                    <button onClick={() => (postType==='photo'?photoRef:videoRef).current?.click()} className="h-28 rounded-xl flex flex-col items-center justify-center" style={{ background:isDark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.02)', border:`2px dashed ${t.cardBorder}` }}>
                      <div className="w-10 h-10 rounded-full flex items-center justify-center mb-2" style={{ background:'rgba(139,92,246,0.15)' }}>
                        <span className="text-sm font-bold" style={{ color:'#8b5cf6' }}>UP</span>
                      </div>
                      <p className="text-xs font-semibold" style={{ color:'#8b5cf6' }}>Upload from Gallery</p>
                      <p className="text-[10px]" style={{ color:t.textMuted }}>Choose file</p>
                    </button>
                  </div>
                )}
              </div>
