...
export default function MapPage() {
  ...
  const filtered = DEMO_WORKERS.filter(w => !search || w.full_name.toLowerCase().includes(search.toLowerCase()) || w.skills.some(s=>s.toLowerCase().includes(search.toLowerCase())));
  ...
  const initMap = useCallback(() => {
    ...
    try {
      ...
      const ui = L.divIcon({ className:'', html:'<div style="width:18px;height:18px;background:#6366f1;border-radius:50%;border:3px solid white;box-shadow:0 0 12px rgba(99,102,241,0.6);"></div>', iconSize:[18,18], iconAnchor:[9,9] });
      L.marker([43.6532, -79.3832], { icon:ui }).addTo(map).bindPopup('<b>You are here</b>');
      ...
      DEMO_WORKERS.forEach(w => {
        ...
        const bg = w.availability==='available'?'linear-gradient(135deg,#22c55e,#16a34a)':'linear-gradient(135deg,#6366f1,#8b5cf6)';
    ...
    <div style={{ margin:'-1rem -0.75rem', height:'calc(100vh - 60px)', position:'relative' }}>
      <div style={{ position:'absolute', top:10, left:10, right:10, zIndex:1000 }}>
        <div className="max-w-lg mx-auto flex gap-2">
          ...
          <button onClick={()=>router.push('/jobplace/providers')} style={{ padding:'10px 16px', borderRadius:16, background:isDark?'rgba(15,15,26,0.93)':'rgba(255,255,255,0.93)', color:t.textSecondary, border:`1px solid ${t.cardBorder}`, fontSize:12, fontWeight:600, cursor:'pointer' }}>📋 List</button>
        </div>
        <div className="max-w-lg mx-auto flex gap-1.5 mt-2">{[5,10,25,50].map(r=>(<button key={r} onClick={()=>setRadius(r)} style={{ padding:'5px 14px', borderRadius:12, background:radius===r?t.accent:(isDark?'rgba(15,15,26,0.85)':'rgba(255,255,255,0.85)'), color:radius===r?'white':t.textSecondary, border:`1px solid ${radius===r?t.accent:t.cardBorder}`, fontSize:11, fontWeight:600, cursor:'pointer' }}>{r}km</button>))}</div>
      </div>
        ...
        <div style={{ position:'absolute', bottom:70, left:8, right:8, zIndex:1001 }}>
          <div className="max-w-md mx-auto" style={{ background:isDark?'rgba(15,15,26,0.96)':'rgba(255,255,255,0.96)', borderRadius:20, padding:16, border:`1px solid ${t.cardBorder}`, boxShadow:'0 -4px 30px rgba(0,0,0,0.2)', backdropFilter:'blur(24px)' }}>
            ...
            <div style={{ display:'flex', gap:12 }}>
              ...
              <div style={{ flex:1 }}><p style={{ fontWeight:700, fontSize:15, margin:0 }}>{selected.full_name} {selected.is_police_verified&&'🛡️'}</p><p style={{ fontSize:12, color:t.textSecondary, margin:'2px 0' }}>★{selected.rating} · {selected.completed_jobs} jobs · Trust: <span style={{ color:selected.trust_score>=80?'#22c55e':'#eab308', fontWeight:700 }}>{selected.trust_score}</span></p><div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>{selected.skills.map((s:string)=><span key={s} style={{ fontSize:10, padding:'2px 8px', borderRadius:20, background:t.accentLight, color:t.accent }}>{s}</span>)}</div></div>
            ...
            <div style={{ display:'flex', gap:8, marginTop:12 }}>
              ...
              <button onClick={handleFav} style={{ padding:'10px 14px', borderRadius:14, background:isFav?'rgba(234,179,8,0.15)':t.surface, color:isFav?'#eab308':t.textSecondary, border:`1px solid ${t.cardBorder}`, fontSize:12, cursor:'pointer' }}>{isFav?'⭐':'☆'}</button>
            </div>
          </div>
        </div>
      )}
